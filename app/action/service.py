"""
Action service — business logic for the Action Catalog.
"""
from sqlalchemy.orm import Session
from app.action import repository as repo
from app.logger.logging import logger


def create_action(db: Session, request, user_id: int) -> dict:
    result = repo.insert_action(db, request, user_id)
    logger.debug(f"Created action '{request.name}' (key={request.action_key})")
    return result


def list_actions(db: Session, status: str | None = None, capability_id: int | None = None,
                 category_id: int | None = None, search_query: str | None = None) -> dict:
    items = repo.fetch_all_actions(db, status=status, capability_id=capability_id,
                                   category_id=category_id, search_query=search_query)
    return {"items": items, "total": len(items)}


def get_action_by_id(db: Session, action_definition_id: int) -> dict | None:
    return repo.fetch_action_by_id(db, action_definition_id)


def update_action(db: Session, action_definition_id: int, request) -> dict:
    result = repo.update_action(db, action_definition_id, request)
    logger.debug(f"Updated action '{action_definition_id}'")
    return result


def update_action_status(db: Session, action_definition_id: int, request) -> dict:
    result = repo.update_action_status(db, action_definition_id, request)
    logger.debug(f"Updated status for action '{action_definition_id}'")
    return result


def delete_action(db: Session, action_definition_id: int) -> bool:
    """Business logic for deleting an action. Returns False if in use."""
    success = repo.delete_action(db, action_definition_id)
    if success:
        logger.info(f"Deleted action '{action_definition_id}'")
    else:
        logger.warning(f"Could not delete action '{action_definition_id}': it is likely in use.")
    return success


def list_actions_grouped(db: Session) -> dict:
    """Get actions grouped by category."""
    return repo.fetch_actions_grouped_by_category(db)
